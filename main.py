...
def print_topic_word_distribution(corpus, number_of_topics, topk, filepath):
    ...
    for k in range(number_of_topics):
        ...
        for i in range(topk):
            ...
            probability = word_index_prob[i][1]
            f.write('\t' + corpus.vocabulary[index] + " : " + str(probability) + '\n')
...
def print_document_topic_distribution(corpus, number_of_topics, topk, filepath):
    ...
    topk = number_of_topics if topk>number_of_topics else topk
    f = open(filepath, "w")
    ...
    for d in range(D):
        ...
        for i in range(topk):
            ...
            probability = topic_index_prob[i][1]
            f.write("\ttopic" + str(index) + " : " + str(probability) + '\n')
...
def main(argv):
    ...
    print "Count of documents:" + str(len(corpus.documents))
    print "Count of words:" + str(corpus.word_count)
